		...
		<?php if ( ! post_password_required() && ! is_attachment() ) : ?>
			<?php if ( is_single() ) { ?>
					...
					<?php if (do_shortcode(get_post_meta( $post->ID, '_debaco_post_intro', true )) != '') { ?>
						<div class="player"><?php echo do_shortcode(get_post_meta( $post->ID, '_debaco_post_intro', true )); ?></div>
					<?php } ?>
				<?php }
			?>
			<?php if ( !is_single() ) { ?>
				<?php if ( has_post_thumbnail() ) { ?>
					<div class="post-thumbnail-wrapper">
						<div class="post-thumbnail">
							<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail($debaco_postthumb); ?></a>
							<?php if(has_category()) { ?>
								<div class="post-category">
									<?php echo get_the_category_list(' '); ?>
								</div> 
						...
						<?php if ( !is_single() ) { ?>
							...
							<?php if( empty( $post->post_title ) ) { ?>
								...
								<span class="post-link"> 
									<a href="<?php the_permalink(); ?>" rel="bookmark"><?php esc_html_e( 'View posts', 'debaco' )?></a>
								</span>
							<?php } ?>
						<?php } ?>
					</div>
					...
					<?php if(get_the_author_meta()!="") { ?>
						<div class="author-info">
							...
							<div class="author-description">
								<h2><?php esc_html_e( 'About the Author:', 'debaco'); printf( '<a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'" rel="author">%s</a>' , get_the_author()); ?></h2>

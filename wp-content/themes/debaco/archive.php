 ...
 * new template file for each specific one. For example, Debaco already
...
<div class="main-container <?php if(isset($debaco_opt['blog_banner']['url']) && ($debaco_opt['blog_banner']['url'])!=''){ echo 'has-image';} ?>">
	...
	<!-- brand logo -->
	<?php 
		if(isset($debaco_opt['inner_brand']) && function_exists('debaco_brands_shortcode') && shortcode_exists( 'ourbrands' ) ){
			if($debaco_opt['inner_brand'] && isset($debaco_opt['brand_logos'][0]) && $debaco_opt['brand_logos'][0]['thumb']!=null) { ?>
				<div class="inner-brands">
					<div class="container">
						<?php if(isset($debaco_opt['inner_brand_title']) && $debaco_opt['inner_brand_title']!=''){ ?>
							<div class="heading-title style1 ">
								<h3><?php echo esc_html( $debaco_opt['inner_brand_title'] ); ?></h3>
							</div>
						<?php } ?>
